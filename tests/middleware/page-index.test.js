...
test('test parse page title with children', () => {
  const pageData = '<h1>title</h1><div id="maincontent"><h1>title2</h1><div><h2>stuff</h2></div></div>';
  const $ = cheerio.load(pageData);

  expect(pageIndex.parsePageHeadings($, 'div')).toEqual(['']);
});

test('test parse meta', () => {
  const pageData = '<meta name="test-meta" content="content result"><h1>title</h1><div id="maincontent"><h1>title2</h1></div>';
...
test('test get connection configuration', () => {
  process.env.MANUAL_USERNAME = 'nameofuser';
  process.env.MANUAL_PASSWORD = 'abcdefgh1234';

  expect(pageIndex.getConnectionConfig()).toStrictEqual({ auth: { password: 'abcdefgh1234', username: 'nameofuser' } });
});
...
test('test init', async () => {
  const returnData = {
    data: ` <main class="nhsuk-main-wrapper app-main-wrapper" id="maincontent" role="main">
          ...
          <ul class="nhsuk-u-nested-list">
            <li><a href="/standards-and-technology/service-standard">NHS service standard</a></li>
              <ul class="nhsuk-u-nested-list">
                ...
                <li><a href="/standards-and-technology/service-standard-points/9-respect-and-protect-users-confidentiality-and-privacy">9. Respect and protect users’ confidentiality and privacy</a></li>
  ...
  const allUrls = [{ data: '<div>some response</div>', request: { path: 'some path' } }];
  ...
  expect(pageIndex.docs.length).toBe(1);
});
